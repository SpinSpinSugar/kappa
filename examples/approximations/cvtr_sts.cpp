/*
   \file cvtr_sts.cpp
   \brief test for translational specific heat at constant volume for STS
   approach
*/
...
int main() {
  ...
  mol_ndens.push_back(
      mixture.Boltzmann_distribution(T, (1. - x_atom) * tot_ndens, MoleculeN2));
  while (T <= 3000.0) {
    // mixture number density
    tot_ndens = p / (kappa::K_CONST_K * T);

    mol_ndens[0] = mixture.Boltzmann_distribution(T, (1. - x_atom) * tot_ndens,
                                                  MoleculeN2);
    ...
    std::cout << std::setw(25) << approx.c_tr(T, MoleculeN2) << std::setw(20)
              << approx.c_tr(T, AtomN) << std::setw(20) << cv_tr;
    std::cout << std::endl;
    ...
    // std::cout << " conservation tests (expected value = 1) " << (test +
    // atom_ndens[0]) / n << std::endl;
