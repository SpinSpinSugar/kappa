/*
   \file basicTest.cpp
   \brief some basic tests - YAML parsing, linear algebra, utility numeric
   functions
*/
...
int main(void) {
  ...
  std::cout << "5! = " << kappa::factorial_table[5]
            << " 69!=" << kappa::factorial_table[69] << "\n";

  auto test_f = [](int i) { return 0.5 + i; };  // find_max_value should give 6
  auto test_f_integration = [](double x) {
    return x;
  };  // integral from 0 to 1 should be 0.5

  std::cout << "int(f)_0^1 = "
            << kappa::integrate_interval(test_f_integration, 0, 1.) << "\n";
