...
int main(int argc, char **argv) {
  ...
  //
  std::cout << "probability_VT = "
            << appr.probability_VT(20000., mol, inter_m_a, 1, -1, 0)
            << std::endl;
  //
  ...
  std::cout << "k_VT FHO: " << appr.k_VT(4000., mol, inter_m_a, 1, -1)
            << std::endl;
  std::cout << "k_VT SSH: "
            << appr.k_VT(4000., mol, inter_m_a, 1, -1,
                         kappa::models_k_vt::model_k_vt_ssh)
            << std::endl;
  // std::cout << "k_VT SSH: " << appr.k_VT(3000., mol, inter_m_a, 1, -1,
  // kappa::models_k_vt::model_k_vt_ssh) << std::endl;
  std::cout << "k_VT FHO: " << appr.k_VT(4000., mol, inter_m_a, 2, -1)
            << std::endl;
  std::cout << "k_VT SSH: "
            << appr.k_VT(4000., mol, inter_m_a, 2, -1,
                         kappa::models_k_vt::model_k_vt_ssh)
            << std::endl;
  // std::cout << "k_VT SSH: " << appr.k_VT(3000., mol, inter_m_a, 2, -1,
  // kappa::models_k_vt::model_k_vt_ssh) << std::endl;
  std::cout << "k_VT FHO: " << appr.k_VT(4000., mol, inter_m_a, 10, -1)
            << std::endl;
  std::cout << "k_VT SSH: "
            << appr.k_VT(4000., mol, inter_m_a, 10, -1,
                         kappa::models_k_vt::model_k_vt_ssh)
            << std::endl;
  // std::cout << "k_VT SSH: " << appr.k_VT(3000., mol, inter_m_a, 10, -1,
  // kappa::models_k_vt::model_k_vt_ssh) << std::endl;
  ...
  for (int i = 0; i < mol.num_vibr_levels[0] - 1; i++) {
    res_kf += i * appr.k_VT(T, mol, inter_m_a, i + 1, -1) * distrib[i + 1] *
              n;  // i+1 -> i
    res_kb += i * appr.k_VT(T, mol, inter_m_a, i + 1, -1) *
              appr.k_bf_VT(T, mol, i + 1, -1) * distrib[i] * n;  // i -> i+1
  }

  for (int i = 1; i < mol.num_vibr_levels[0]; i++) {
    res_kf += i * appr.k_VT(T, mol, inter_m_a, i, -1) *
              appr.k_bf_VT(T, mol, i, -1) * distrib[i - 1] * n;  // i-1 -> i
    res_kb +=
        i * appr.k_VT(T, mol, inter_m_a, i, -1) * distrib[i] * n;  // i -> i-1
  }
