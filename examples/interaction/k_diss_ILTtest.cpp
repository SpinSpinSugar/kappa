/*
   ...
   \brief Computing dissociation rate coefficient k_diss according to Inverse
   Laplace Transform (ILT)
*/
...
void calculate(Molecule m, Atom a, Approximation appr) {
  ...
  // std::vector<std::string> non_TM_model_names = {"RS, center-of-mass line
  // energy + vibr energy", "RS, full energy + vibr energy",
  //                                                "VSS, center-of-mass line
  //                                                energy + vibr energy", "VSS,
  //                                                full energy + vibr energy",
  ...
  for (auto T : T_vals) {
    for (auto i : i_vals) {
      ...
      if (m.name == "O2") {
        ...
        c2 = 4e-19 * i * i * i * i + 5.24e-19 * i * i * i - 7.41e-17 * i * i +
             6.42e-15 * i + 7.3e-14;
        outf << std::setw(20)
             << 7.16e-2 * pow(T, -0.25) * c2 *
                    exp((m.vibr_energy[0][i] - c1) / (K_CONST_K * T));
        outf << endl;
      }
    }
  }
