...
int main(int argc, char **argv) {
  std::cout << "Start Test for k_exch coefficients, loading particle data"
            << endl;
  ...
  std::vector<kappa::models_k_exch> model_vals = {
      models_k_exch::model_k_exch_arrh_scanlon,
      models_k_exch::model_k_exch_arrh_park};
  double res;
  int j = 0;
  ...
  for (auto exch_model : model_vals) {
    for (auto T : T_vals) {
      for (auto i : i_vals) {
        ...
        outf << std::setw(15)
             << ApproximationTest.k_exch(
                    T, Molecule1, Atom1, inter, i,
                    models_k_exch::model_k_exch_arrh_scanlon)
             << std::setw(20);
        outf << std::setw(15)
             << ApproximationTest.k_exch(T, Molecule1, Atom1, inter, i,
                                         models_k_exch::model_k_exch_arrh_park)
             << std::setw(20);
        outf << std::endl;
      }
