/*
   \file k_dissTest.cpp
   \brief Computing dissociation rate coefficients k_diss according to several
   models.
*/
...
void calculate(Molecule m, Atom a, Approximation appr) {
  std::cout << "Loading interaction parameters for " << m.name << "+" << a.name
            << endl;
  ...
  std::vector<kappa::models_k_diss> ALL_model_vals = {
  ...
  std::vector<std::string> non_TM_model_names = {
      "RS, center-of-mass line energy + vibr energy",
      "RS, full energy + vibr energy",
      "VSS, center-of-mass line energy + vibr energy",
      "VSS, full energy + vibr energy",
      "Phys4Entry approximation",
      "ILT"};

  std::vector<kappa::models_k_diss> non_TM_model_vals = {
      models_k_diss::model_k_diss_rs_thresh_cmass_vibr,
      models_k_diss::model_k_diss_rs_thresh_vibr,
      models_k_diss::model_k_diss_vss_thresh_cmass_vibr,
      models_k_diss::model_k_diss_vss_thresh_vibr,
  ...
  // std::vector<std::string> TM_model_names = {"Treanor-Marrone, U=inf,
  // Scanlon", "Treanor-Marrone, U=D/6K, Scanlon", "Treanor-Marrone, U=3T,
  // Scanlon"}; std::vector<kappa::models_k_diss> TM_model_vals =
  // {model_k_diss_tm_infty_arrh_scanlon, model_k_diss_tm_D6k_arrh_scanlon,
  // model_k_diss_tm_3T_arrh_scanlon};

  // std::vector<std::string> TM_model_names = {"Treanor-Marrone, U=inf, Park",
  // "Treanor-Marrone, U=D/6K, Park", "Treanor-Marrone, U=3T, Park"};
  // std::vector<kappa::models_k_diss> TM_model_vals =
  // {models_k_diss::model_k_diss_tm_infty_arrh_park,
  // models_k_diss::model_k_diss_tm_D6k_arrh_park,
  // models_k_diss::model_k_diss_tm_3T_arrh_park};
    ...
    for (auto i : i_vals) {
      for (auto diss_model : ALL_model_vals) {
        ...
        outf << std::setw(15)
             ...
             << std::setw(20);
        outf << std::setw(15)
             << appr.k_diss(T, m, inter, i, models_k_diss::model_k_diss_ilt)
             << std::setw(20);
        outf << std::endl;
        counter++;
      }
...
int main(int argc, char **argv) {
  std::cout << "Start Test for k_diss coefficients, loading particle data"
            << endl;
