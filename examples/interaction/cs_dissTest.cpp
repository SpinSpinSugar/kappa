...
void calculate(Molecule m, Atom a, Approximation appr) {
  ...
  std::vector<double> t_vals = {
      1000.,   2000.,   4000.,   5000.,   10000.,  15000.,  20000.,
      25000.,  30000.,  45000.,  50000.,  55000.,  60000.,  65000.,
  ...
  std::vector<std::string> cs_model_names = {
      "RS",
  ...
  std::vector<kappa::models_cs_diss> cs_model_vals = {
      models_cs_diss::model_cs_diss_rs_thresh,
  ...
  for (auto i : i_vals) {
    ...
    for (auto diss_model : cs_model_vals) {
      outf << cs_model_names[counter] << ",i=" << i << "("
           << m.vibr_energy[0][i] / K_CONST_EV << " eV);";
      counter++;
    }
    outf << ";";
  }
...
int main(int argc, char **argv) {
  std::cout
      << "Start Test for dissociation cross sections, loading particle data"
      << endl;
