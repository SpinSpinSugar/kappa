/*
  \file mixture-sts-basic_boltzman.cpp
  \brief Compute Boltzmann distribution and operate some conversions.
*/
...
int main(int argc, char **argv) {
  ...
  int _NS = 0;    // number of species
  int _nMol = 0;  // number of molecules
  int _nAt = 0;   // number of atoms

  std::vector<std::string> _speciesNames;
  std::vector<std::pair<std::string, double>> composition;

  // mixture selection
  std::string _mixtureName = "N2";
  // std::string _mixtureName = "air5";
  // std::string _mixtureName = "N2ions";
  // std::string _mixtureName = "O2";
  ...
  std::ifstream mixfile(
      (m_libPath + "mixtures/" + _mixtureName + ".mix").c_str());
  ...
  */
  ...
  for (auto mol : molecules) {
    mol_ndens.push_back(
        mixture.Boltzmann_distribution(T0, (1. - x_N) * tot_ndens, mol, 0));
  }
  ...
  // conversion from std::vec to arma::vec
  ...
  std::cout << " density = " << density << " at temperature = " << T0
            << " and pressure = " << p0 << std::endl;
