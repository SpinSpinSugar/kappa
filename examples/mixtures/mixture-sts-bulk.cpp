...
int main(int argc, char **argv) {
  ...
  mol_ndens.push_back(mixture.Boltzmann_distribution(
      T_vals[0], x_N2 * 101325.0 / (K_CONST_K * T_vals[0]), mol));  // N2
  // mol_ndens.push_back(mixture.Boltzmann_distribution(T_vals[0], x_O2 *
  // 101325.0 / (K_CONST_K * T_vals[0]), mol)); // O2

  std::ofstream outf;
  outf.open(output_dir + "/TRANSPORT_COEFFICIENTS/bulk_viscosity/" + mol.name +
            "_xat_" + std::to_string(x_N2) + ".txt");
  // outf.open(output_dir + "/TRANSPORT_COEFFICIENTS/bulk_viscosity/" + mol.name
  // + "_xat_" + std::to_string(x_O2) + ".txt");
  ...
  for (auto T : T_vals) {
    ...
    mol_ndens[0] = mixture.Boltzmann_distribution(
        T, x_N2 * 101325.0 / (K_CONST_K * T), mol);
    atom_ndens[0] = x_N * 101325.0 / (K_CONST_K * T);
    // mol_ndens[0] = mixture.Boltzmann_distribution(T, x_O2 * 101325.0 /
    // (K_CONST_K * T), mol); atom_ndens[0] = x_O * 101325.0 / (K_CONST_K * T);

    // Attention: looks like the perturbation is non-zero is not defined! Better
  ...
  // std::cout << mol_ndens[0] / 101325.0 * (K_CONST_K * T_vals[0]) <<
  // std::endl; std::cout << "ATOMIC MOLAR FRACTIONS"  << std::endl; std::cout
  // << atom_ndens[0] / 101325.0 * (K_CONST_K * T_vals[0])  << std::endl;
  // std::cout << "MOLECULAR MASS FRACTIONS"  << std::endl;
  // std::cout << mol_ndens[0] / 101325.0 * (K_CONST_K * T_vals[0])  <<
  // std::endl; std::cout << "ATOMIC MASS FRACTIONS"  << std::endl; std::cout <<
  // atom_ndens[0] / 101325.0 * (K_CONST_K * T_vals[0])  << std::endl;
