/*
   ...
   \brief Generate database of transport coefficients for air5 mixture in STS
   approach.
*/
...
int main(int argc, char **argv) {
  ...
  for (int n = 0; n < n_samples; n++) {
    ...
    Xi[3][n] = x4;
    Xi[4][n] = x5;
    // std::cout << Xi[0][n] << " " << Xi[1][n] << " " << Xi[2][n] << " " <<
    // Xi[3][n] << " " << Xi[4][n] << std::endl;
  }
  fclose(initial);
  ...
  mol_ndens.push_back(mixture.Boltzmann_distribution(
      T_vals[0], P_vals[0] / (kappa::K_CONST_K * T_vals[0]), N2));
  mol_ndens.push_back(mixture.Boltzmann_distribution(
      T_vals[0], P_vals[0] / (kappa::K_CONST_K * T_vals[0]), O2));
  mol_ndens.push_back(mixture.Boltzmann_distribution(
      T_vals[0], P_vals[0] / (kappa::K_CONST_K * T_vals[0]), NO));
  ...
  // for (i=0; i<124; i++) {
  //   for (int j=0; j<124; j++) {
  //     outf << std::setw(12) << std::scientific << "Dij["<<i<<"]["<<j<<"]"; //
  //     << std::endl;
  //   }
  // }
  outf << std::endl;
  ...
  for (auto T : T_vals) {
    for (auto P : P_vals) {
      ...
      for (int lv = 0; lv < n_samples; lv++) {
        ...
        std::cout << lv << " " << x_N2 << " " << x_O2 << " " << x_NO << " "
                  << x_N << " " << x_O << std::endl;

        mol_ndens[0] =
            mixture.Boltzmann_distribution(T, (1. - x_N2) * tot_ndens, N2);
        mol_ndens[1] =
            mixture.Boltzmann_distribution(T, (1. - x_O2) * tot_ndens, O2);
        mol_ndens[2] =
            mixture.Boltzmann_distribution(T, (1. - x_NO) * tot_ndens, NO);
        ...
        mixture.compute_transport_coefficients(
            T, mol_ndens, atom_ndens, 0, models_omega::model_omega_rs, 0.);
        // mixture.compute_transport_coefficients(T, mol_ndens, atom_ndens, 0,
        // models_omega::model_omega_esa, 0.);
