...
int main(int argc, char **argv) {
  ...
  // kappa::Molecule mol("NO", true, false, particle_source);
  ...
  double x_N = 0.10;
  double x_O = 0.10;
  double pressure = 101325.;  // 27358.000000; //101325.;
  ...
  mol_ndens.push_back(mixture.Boltzmann_distribution(
      T_vals[0], x_N2 * pressure / (K_CONST_K * T_vals[0]), mol));  // N2
  // mol_ndens.push_back(mixture.Boltzmann_distribution(T_vals[0], x_O2 *
  // pressure / (K_CONST_K * T_vals[0]), mol)); // O2

  std::ofstream outf;
  outf.open(output_dir + "/TRANSPORT_COEFFICIENTS/shear_viscosity/" + mol.name +
            "_xat_" + std::to_string(x_N2) + ".txt");
  // outf.open(output_dir + "/TRANSPORT_COEFFICIENTS/shear_viscosity/" +
  // mol.name + "_xat_" + std::to_string(x_O2) + ".txt");
  ...
  for (auto T : T_vals) {
    ...
    mol_ndens[0] = mixture.Boltzmann_distribution(
        T, x_N2 * pressure / (K_CONST_K * T), mol);
    atom_ndens[0] = x_N * pressure / (K_CONST_K * T);
    // mol_ndens[0] = mixture.Boltzmann_distribution(T, x_O2 * pressure /
    // (K_CONST_K * T), mol); atom_ndens[0] = x_O * pressure / (K_CONST_K * T);

    // mixture.compute_transport_coefficients(T, mol_ndens, atom_ndens);
    mixture.compute_transport_coefficients(T, mol_ndens, atom_ndens, 0,
                                           models_omega::model_omega_rs, 0.);
  ...
  std::cout << std::setw(20)
            << " shear viscosity = " << mixture.get_shear_viscosity()
            << std::endl;
  std::cout << std::setw(20)
            << " bulk viscosity = " << mixture.get_bulk_viscosity()
            << std::endl;
  std::cout << std::setw(20)
            << " thermal conductivity = " << mixture.get_thermal_conductivity()
            << std::endl;
  arma::vec thd;  // thermo-diffusion arma vector
  thd = mixture.get_thermodiffusion();
  std::cout << " thermal diffusion = " << mixture.get_thermodiffusion()
            << std::endl;
