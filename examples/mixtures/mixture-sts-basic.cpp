...
int main(int argc, char **argv) {
  ...
  mol_ndens.push_back(mixture.Boltzmann_distribution(
      T_vals[0], 101325.0 / (K_CONST_K * T_vals[0]), N2));
  mol_ndens.push_back(mixture.Boltzmann_distribution(
      T_vals[0], 101325.0 / (K_CONST_K * T_vals[0]), O2));
  mol_ndens.push_back(mixture.Boltzmann_distribution(
      T_vals[0], 101325.0 / (K_CONST_K * T_vals[0]), NO));
  ...
  for (auto T : T_vals) {
    mol_ndens[0] =
        mixture.Boltzmann_distribution(T, 101325.0 / (K_CONST_K * T), N2);
    mol_ndens[1] =
        mixture.Boltzmann_distribution(T, 101325.0 / (K_CONST_K * T), O2);
    mol_ndens[2] =
        mixture.Boltzmann_distribution(T, 101325.0 / (K_CONST_K * T), NO);
    std::cout << std::setw(20) << T;
    std::cout << std::setw(20)
              << mol_ndens[0][0] / (101325.0 / (K_CONST_K * T));
    std::cout << std::setw(20)
              << mol_ndens[1][0] / (101325.0 / (K_CONST_K * T));
    std::cout << std::setw(20)
              << mol_ndens[2][0] / (101325.0 / (K_CONST_K * T));
    std::cout << std::endl;
  }
