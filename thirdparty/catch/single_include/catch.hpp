...
// auto_ptr) CATCH_CONFIG_CPP11_SHUFFLE : is std::shuffle supported?
...
#endif
#if defined(CATCH_INTERNAL_CONFIG_CPP11_OVERRIDE) && \
    !defined(CATCH_CONFIG_CPP11_NO_OVERRIDE) &&      \
    !defined(CATCH_CONFIG_CPP11_OVERRIDE) && !defined(CATCH_CONFIG_NO_CPP11)
#define CATCH_CONFIG_CPP11_OVERRIDE
#endif
#if defined(CATCH_INTERNAL_CONFIG_CPP11_UNIQUE_PTR) && \
    !defined(CATCH_CONFIG_CPP11_NO_UNIQUE_PTR) &&      \
    !defined(CATCH_CONFIG_CPP11_UNIQUE_PTR) && !defined(CATCH_CONFIG_NO_CPP11)
#define CATCH_CONFIG_CPP11_UNIQUE_PTR
#endif
// Use of __COUNTER__ is suppressed if __JETBRAINS_IDE__ is #defined (meaning
// we're being parsed by a JetBrains IDE for analytics) because, at time of
// writing, __COUNTER__ is not properly handled by it. This does not affect
// compilation
...
#endif
#if defined(CATCH_INTERNAL_CONFIG_CPP11_SHUFFLE) && \
    !defined(CATCH_CONFIG_CPP11_NO_SHUFFLE) &&      \
    !defined(CATCH_CONFIG_CPP11_SHUFFLE) && !defined(CATCH_CONFIG_NO_CPP11)
#define CATCH_CONFIG_CPP11_SHUFFLE
#endif
#if defined(CATCH_INTERNAL_CONFIG_CPP11_TYPE_TRAITS) && \
    !defined(CATCH_CONFIG_CPP11_NO_TYPE_TRAITS) &&      \
...
struct DecomposedExpression {
  ...
  virtual bool isBinaryExpression() const { return false; }
...
struct MatchAllOf : MatcherBase<ArgT> {
  virtual bool match(ArgT const &arg) const CATCH_OVERRIDE {
    for (std::size_t i = 0; i < m_matchers.size(); ++i) {
      if (!m_matchers[i]->match(arg)) return false;
    }
    return true;
  }
...
struct MatchAnyOf : MatcherBase<ArgT> {
  virtual bool match(ArgT const &arg) const CATCH_OVERRIDE {
    for (std::size_t i = 0; i < m_matchers.size(); ++i) {
      if (m_matchers[i]->match(arg)) return true;
    }
    return false;
  }
  virtual std::string describe() const CATCH_OVERRIDE {
...
template <typename T>
inline Impl::MatchAllOf<T> AllOf(Impl::MatcherBase<T> const &m1,
                                 Impl::MatcherBase<T> const &m2) {
...
template <typename T>
inline Impl::MatchAnyOf<T> AnyOf(Impl::MatcherBase<T> const &m1,
                                 Impl::MatcherBase<T> const &m2) {
...
class ExpressionLhs : public DecomposedExpression {
 public:
  ExpressionLhs(ResultBuilder &rb, T lhs)
      : m_rb(rb), m_lhs(lhs), m_truthy(false) {}

  ExpressionLhs &operator=(const ExpressionLhs &);

  template <typename RhsT>
  BinaryExpression<T, Internal::IsEqualTo, RhsT const &> operator==(
      RhsT const &rhs) {
    return captureExpression<Internal::IsEqualTo>(rhs);
  }

  template <typename RhsT>
  BinaryExpression<T, Internal::IsNotEqualTo, RhsT const &> operator!=(
      RhsT const &rhs) {
    return captureExpression<Internal::IsNotEqualTo>(rhs);
  }

  template <typename RhsT>
  BinaryExpression<T, Internal::IsLessThan, RhsT const &> operator<(
      RhsT const &rhs) {
    return captureExpression<Internal::IsLessThan>(rhs);
  }

  template <typename RhsT>
  BinaryExpression<T, Internal::IsGreaterThan, RhsT const &> operator>(
      RhsT const &rhs) {
    return captureExpression<Internal::IsGreaterThan>(rhs);
  }

  template <typename RhsT>
  BinaryExpression<T, Internal::IsLessThanOrEqualTo, RhsT const &> operator<=(
      RhsT const &rhs) {
...
class BinaryExpression : public DecomposedExpression {
 public:
  ...
  void endExpression() const {
    m_rb.setResultType(Internal::compare<Op>(m_lhs, m_rhs))
        .endExpression(*this);
  }

  virtual bool isBinaryExpression() const CATCH_OVERRIDE { return true; }
...
class CompositeGenerator {
 ...
 private:
  void move(CompositeGenerator &other) {
    m_composed.insert(m_composed.end(), other.m_composed.begin(),
...
class Approx {
 public:
  ...
  friend bool operator==(const T &lhs, Approx const &rhs) {
    ...
    bool relativeOK =
        std::fabs(lhs_v - rhs.m_value) <
        rhs.m_epsilon * (rhs.m_scale +
                         (std::max)(std::fabs(lhs_v), std::fabs(rhs.m_value)));
    if (relativeOK) {
      return true;
    }
  ...
  friend bool operator==(double lhs, Approx const &rhs) {
    ...
    bool relativeOK =
        ...
        rhs.m_epsilon *
            (rhs.m_scale + (std::max)(std::fabs(lhs), std::fabs(rhs.m_value)));
    if (relativeOK) {
      return true;
    }
...
struct StringHolder : MatcherBase<NSString *> {
  ...
  virtual bool match(NSString *arg) const CATCH_OVERRIDE { return false; }
...
// compiler generated methods CLARA_CONFIG_CPP11_OVERRIDE : is override
// supported? CLARA_CONFIG_CPP11_UNIQUE_PTR : is unique_ptr supported (otherwise
// use auto_ptr)
...
class Parser {
 ...
 public:
  ...
  Mode handleOpt(std::size_t i, char c, std::string const &arg,
                 std::vector<Token> &tokens) {
    if (std::string(":=\0", 3).find(c) == std::string::npos) return mode;
...
inline void setRngSeed(ConfigData &config, std::string const &seed) {
  if (seed == "time") {
    ...
    if (ss.fail())
      throw std::runtime_error(
          "Argument to --rng-seed should be the word 'time' or a number");
  }
}
 ...
 public:
  Text(std::string const &_str, TextAttributes const &_attr = TextAttributes())
    ...
    while (it != strEnd) {
      ...
      if (itNewLine != itEnd) itEnd = itNewLine;

      if (itEnd != strEnd) {
        bool foundWrapPoint = false;
        iterator findIt = itEnd;
        do {
          if (wrappableAfterChars.find(*findIt) != std::string::npos &&
              findIt != itEnd) {
            itEnd = findIt + 1;
            itNext = findIt + 1;
            foundWrapPoint = true;
          } else if (findIt > it &&
                     wrappableBeforeChars.find(*findIt) != std::string::npos) {
            itEnd = findIt;
            itNext = findIt;
            foundWrapPoint = true;
          } else if (wrappableInsteadOfChars.find(*findIt) !=
                     std::string::npos) {
            itNext = findIt + 1;
            itEnd = findIt;
            foundWrapPoint = true;
        ...
        if (!foundWrapPoint) {
          ...
          while (itEnd > it && wrappableInsteadOfChars.find(*(itEnd - 1)) !=
                                   std::string::npos)
            --itEnd;
        }
      }
...
struct IReporterRegistry {
  ...
  virtual IStreamingReporter *create(
      std::string const &name, Ptr<IConfig const> const &config) const = 0;
  virtual FactoryMap const &getFactories() const = 0;
  virtual Listeners const &getListeners() const = 0;
};

Ptr<IStreamingReporter> addReporter(
...
class TrackerBase : public ITracker {
 ...
 public:
  ...
  virtual ITracker *findChild(NameAndLocation const &nameAndLocation)
      CATCH_OVERRIDE {
...
class IndexTracker : public TrackerBase {
 ...
 public:
  IndexTracker(NameAndLocation const &nameAndLocation, TrackerContext &ctx,
      ...
      : TrackerBase(nameAndLocation, ctx, parent), m_size(size), m_index(-1) {}
...
inline ITracker &TrackerContext::startRun() {
  m_rootTracker = new SectionTracker(
      NameAndLocation("{root}", CATCH_INTERNAL_LINEINFO), *this, CATCH_NULL);
  m_currentTracker = CATCH_NULL;
  m_runState = Executing;
...
class RunContext : public IResultCapture, public IRunner {
 ...
 private:  // IResultCapture
  virtual void assertionEnded(AssertionResult const &result) {
    ...
    // Reset working state
    m_lastAssertionInfo =
        AssertionInfo(std::string(), m_lastAssertionInfo.lineInfo,
  ...
  virtual void exceptionEarlyReported() { m_shouldReportUnexpected = false; }
 ...
 private:
  void runCurrentTest(std::string &redirectedCout,
    ...
    try {
      m_lastAssertionInfo =
          AssertionInfo("TEST_CASE", testCaseInfo.lineInfo, std::string(),
...
Ptr<IStreamingReporter> makeReporter(Ptr<Config> const &config) {
  ...
  if (reporters.empty()) reporters.push_back("console");
...
class RegistryHub : public IRegistryHub, public IMutableRegistryHub {
 ...
 public:  // IMutableRegistryHub
  ...
  virtual void registerListener(Ptr<IReporterFactory> const &factory)
      CATCH_OVERRIDE {
...
class Context : public IMutableContext {
 ...
 public:
  virtual ~Context() { deleteAllValues(m_generatorsByTestName); }
...
std::string toString(int value) {
  std::ostringstream oss;
  oss << value;
  if (value > Detail::hexThreshold) oss << " (0x" << std::hex << value << ')';
  return oss.str();
}
...
std::string capturedExpressionWithSecondArgument(
    std::string const &capturedExpression, std::string const &secondArg) {
...
AssertionResult ResultBuilder::build() const { return build(*this); }
...
void ResultBuilder::setExceptionGuard() { m_guardException = true; }
void ResultBuilder::unsetExceptionGuard() { m_guardException = false; }
...
void TagAliasRegistry::add(std::string const &alias, std::string const &tag,
  ...
  if (!startsWith(alias, "[@") || !endsWith(alias, ']')) {
    std::ostringstream oss;
    oss << Colour(Colour::Red) << "error: tag alias, \"" << alias
  ...
  if (!m_registry.insert(std::make_pair(alias, TagAlias(tag, lineInfo)))
    ...
    oss << Colour(Colour::Red) << "error: tag alias, \"" << alias
        ...
        << '\n'
        << Colour(Colour::Red) << "\tRedefined at " << Colour(Colour::FileName)
        << lineInfo << '\n';
    throw std::domain_error(oss.str().c_str());
  }
}
...
Ptr<IStreamingReporter> addReporter(
  ...
  if (existingReporter) {
    ...
    if (!multi) {
      ...
      if (existingReporter) multi->add(existingReporter);
    } else
...
class JunitReporter : public CumulativeReporterBase {
 public:
  ...
  virtual void testCaseStarting(TestCaseInfo const &testCaseInfo)
      CATCH_OVERRIDE {
...
#define CATCH_REQUIRE(expr)                   \
  INTERNAL_CATCH_TEST_NO_TRY("CATCH_REQUIRE", \
...
#else
#define CATCH_REQUIRE(expr) \
  INTERNAL_CATCH_TEST("CATCH_REQUIRE", Catch::ResultDisposition::Normal, expr)
#define CATCH_REQUIRE_FALSE(expr)                                             \
  INTERNAL_CATCH_TEST(                                                        \
      "CATCH_REQUIRE_FALSE",                                                  \
      Catch::ResultDisposition::Normal | Catch::ResultDisposition::FalseTest, \
...
#define CATCH_REQUIRE_THROWS_WITH(expr, matcher)     \
  INTERNAL_CATCH_THROWS("CATCH_REQUIRE_THROWS_WITH", \
...
#define CATCH_CHECK_THROWS_WITH(expr, matcher)                                \
  INTERNAL_CATCH_THROWS("CATCH_CHECK_THROWS_WITH",                            \
...
#define CATCH_REQUIRE_THAT(arg, matcher)                    \
  INTERNAL_CHECK_THAT_NO_TRY("CATCH_REQUIRE_THAT", matcher, \
...
#define CATCH_CAPTURE(msg) \
  INTERNAL_CATCH_INFO("CATCH_CAPTURE", #msg " := " << Catch::toString(msg))
...
#define CATCH_FAIL(msg)                                               \
  INTERNAL_CATCH_MSG("CATCH_FAIL", Catch::ResultWas::ExplicitFailure, \
                     Catch::ResultDisposition::Normal, msg)
...
#if defined(CATCH_CONFIG_FAST_COMPILE)
#define REQUIRE(expr) \
  INTERNAL_CATCH_TEST_NO_TRY("REQUIRE", Catch::ResultDisposition::Normal, expr)
...
#else
#define REQUIRE(expr) \
  INTERNAL_CATCH_TEST("REQUIRE", Catch::ResultDisposition::Normal, expr)
#define REQUIRE_FALSE(expr)                                                   \
  INTERNAL_CATCH_TEST(                                                        \
      "REQUIRE_FALSE",                                                        \
      Catch::ResultDisposition::Normal | Catch::ResultDisposition::FalseTest, \
...
#define REQUIRE_THROWS_WITH(expr, matcher)     \
  INTERNAL_CATCH_THROWS("REQUIRE_THROWS_WITH", \
                        Catch::ResultDisposition::Normal, matcher, expr)
...
#define CHECK_THROWS_WITH(expr, matcher)                                      \
  INTERNAL_CATCH_THROWS("CHECK_THROWS_WITH",                                  \
                        Catch::ResultDisposition::ContinueOnFailure, matcher, \
                        expr)
...
#define REQUIRE_THAT(arg, matcher)                    \
  INTERNAL_CHECK_THAT_NO_TRY("REQUIRE_THAT", matcher, \
                             Catch::ResultDisposition::Normal, arg)
#else
#define REQUIRE_THAT(arg, matcher)             \
...
#define CAPTURE(msg) \
  INTERNAL_CATCH_INFO("CAPTURE", #msg " := " << Catch::toString(msg))
#define SCOPED_CAPTURE(msg) \
  INTERNAL_CATCH_INFO("CAPTURE", #msg " := " << Catch::toString(msg))
