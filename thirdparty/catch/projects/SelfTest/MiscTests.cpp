...
TEST_CASE("random SECTION tests", "[.][sections][failing]") {
  ...
  SECTION("s2", "not equal") { REQUIRE(a != b); }
}
...
TEST_CASE("more nested SECTION tests", "[sections][failing][.]") {
  ...
  SECTION("s1", "doesn't equal") {
    SECTION("s2", "equal") { REQUIRE(a == b); }

    SECTION("s3", "not equal") { REQUIRE(a != b); }
    SECTION("s4", "less than") { REQUIRE(a < b); }
  }
}
...
TEST_CASE("checkedIf", "") { REQUIRE(testCheckedIf(true)); }
...
TEST_CASE("checkedElse", "") { REQUIRE(testCheckedElse(true)); }
...
TEST_CASE("xmlentitycheck", "") {
  SECTION(
      "embedded xml",
      "<test>it should be possible to embed xml characters, such as <, \" or "
      "&, or even whole <xml>documents</xml> within an attribute</test>") {
    SUCCEED("");  // We need this here to stop it failing due to no tests
  }
  SECTION("encoded chars",
          "these should all be encoded: &&&\"\"\"<<<&\"<<&\"") {
    SUCCEED("");  // We need this here to stop it failing due to no tests
  }
}
...
//    char line[200];
//    FILE* output = popen("./CatchSelfTest ./failing/matchers/StartsWith",
//    "r"); while ( fgets(line, 199, output) )
//        std::cout << line;
//}
...
TEST_CASE("not allowed", "[!throws]") {
  // This test case should not be included if you run with -e on the command
  // line
  SUCCEED("");
}

// TEST_CASE( "Is big endian" ) {
//     CHECK( Catch::Detail::Endianness::which() ==
//     Catch::Detail::Endianness::Little );
// }
...
TEST_CASE("toString on const wchar_t const pointer returns the string contents",
          "[toString]") {
...
TEST_CASE("toString on const wchar_t pointer returns the string contents",
          "[toString]") {
...
TEST_CASE("toString on wchar_t const pointer returns the string contents",
          "[toString]") {
...
inline std::string encode(
    ...
    Catch::XmlEncode::ForWhat forWhat = Catch::XmlEncode::ForTextNodes) {
  std::ostringstream oss;
...
TEST_CASE("XmlEncode") {
  ...
  SECTION("empty string") { REQUIRE(encode("") == ""); }
  ...
  SECTION("string with quotes") {
    ...
    REQUIRE(encode(stringWithQuotes, Catch::XmlEncode::ForAttributes) ==
...
TEST_CASE("# A test name that starts with a #") { SUCCEED("yay"); }
