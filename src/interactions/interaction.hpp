/*!
    ...
    \brief The Interaction class loads and stores data for particle-particle
   interactions
 */
...
class Interaction {
 public:
  ...
  // @param const Particle &Particle1   - first particle participating in the
  // interaction
  // @param const Particle &Particle2   - second particle participating in the
  // interaction
  // @param const std::string &filename - path to the interaction.yaml database
  // file
  Interaction(const Particle &Particle1, const Particle &Particle2,
              const std::string &filename = "interaction.yaml");
  ...
  // Charge of the first particle participating in the interaction (expressed in
  ...
  // Is there data of the VSS potential for this interaction ("true" if present,
  ...
  // @param d the diameter of the collision is expressed in terms of the
  // relative velocity of the colliding particles g, g_{ref} reference speed
  double vss_omega;
  ...
  // d_vss = d_ref * (g_ref / g)^(vss_omega - 0.5) = vss_c_d * g^(0.5 -
  // vss_omega)
  double vss_c_d;

  // \pi * d^2 = vss_{c,cs} g^(1 - 2\omega)
  // Auxiliary value vss_{c,cs}
  // The cross section for the collision is expressed in terms of it by the
  // formula: \pi * d_vss^2 = pi d_ref^2 * (g_ref / g)^(2 * vss_omega - 1) =
  // vss_c_cs * g^(1 - 2 * vss_omega)
  double vss_c_cs;

  // Type of interaction, which can be:
  // @param kappa::interaction_types::interaction_neutral_neutral  - interaction
  // of uncharged particles
  // @param kappa::interaction_types::interaction_neutral_ion      - interaction
  // of neutral particles and ions
  // @param kappa::interaction_types::interaction_neutral_electron - interaction
  // of neutral particles and electrons
  // @param kappa::interaction_types::interaction_charged_charged  - interaction
