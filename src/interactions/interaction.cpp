...
Interaction::Interaction(const Particle &Particle1, const Particle &Particle2,
  ...
  } else if (((Particle1.charge != 0) && (Particle2.charge == 0)) ||
             ((Particle1.charge == 0) && (Particle2.charge != 0))) {
  ...
  double LJe =
      sqrt(Particle1.LennardJones_epsilon * Particle2.LennardJones_epsilon) *
      pow(Particle1.diameter * Particle2.diameter, 3) / pow(cd, 6);

  data.insert(std::pair<string, double>(
      "collision mass",
      Particle1.mass * Particle2.mass / (Particle1.mass + Particle2.mass)));
  collision_mass =
      Particle1.mass * Particle2.mass / (Particle1.mass + Particle2.mass);
  ...
  data.insert(std::pair<string, double>("Lennard-Jones epsilon",
                                        LJe));  // TODO check if exists
  epsilon = LJe;
...
void Interaction::readData(const string &name, const string &filename) {
  ...
  if (!file[name]) {
    try {
      std::string error_string =
          "No data found for " + name + " interaction in the database";
    }
    // throw DataNotFoundException(error_string);	}
    catch (int num) {
      // int exceptions thrown in the above try block will be handled here
      cerr << "No data found for " + name + " interaction in the database "
           << num << endl;
    }
  }
  ...
  for (YAML::const_iterator it = interaction.begin(); it != interaction.end();
       ++it) {
    if (!it->second.IsNull()) {
      if (it->second.IsSequence()) {
        // vector_data.insert(std::pair<string,
        // arma::vec>(it->first.as<std::string>(),));
        tmp = it->second.as<std::vector<double>>();
        for (int i = 0; i < tmp.size(); i++) {
          data.insert(std::pair<string, double>(
              "_" + it->first.as<std::string>() + "_" + std::to_string(i),
              tmp[i]));
        }
      } else {
        data.insert(std::pair<string, double>(it->first.as<std::string>(),
                                              it->second.as<double>()));
...
const double &kappa::Interaction::operator[](const std::string &name) const {
  try {
    ...
    std::string error_string =
        "No " + name + " interaction parameter found for " + particle1_name +
        "+" + particle2_name + " interaction";
...
const double &kappa::Interaction::operator[](const char *name) const {
  ...
  try {
    ...
    std::string error_string =
        "No " + conv_str + " interaction parameter found for " +
        particle1_name + "+" + particle2_name + " interaction";
    throw kappa::DataNotFoundException(error_string.c_str());
  }
}
