...
Molecule::Molecule(const std::string &name, bool anharmonic_spectrum,
                   bool rigid_rotator, const std::string &filename)
    : Particle(name, filename) {
  ...
  for (e = 0; e < num_electron_levels; e++) {
    ...
    while (tmp < diss_energy[e] - electron_energy[e]) {
      // Morse potential (anharmonic oscillator model), eq. 1.3, Kustova &
      // Nagnibeda, 2009.
      tmp =
          K_CONST_H * K_CONST_C *
          (vibr_frequency[e] * (i + 0.5) - level_we_xe * (i + 0.5) * (i + 0.5) +
    ...
    if (rigid_rotator == true) {
      ...
      for (i = 0; i < num_vibr_levels[e]; i++) {
        ...
        while (tmp + vibr_energy[e][i] < diss_energy[e] - electron_energy[e]) {
          tmp = K_CONST_H * K_CONST_C *
                ((rot_be[e] - rot_ae[e] * (i + 0.5)) * j * (j + 1));
  ...
  // formula p. 10
  characteristic_vibr_temperatures =
      K_CONST_H * K_CONST_C * vibr_frequency / K_CONST_K;
...
void Molecule::readData(const std::string &name, const std::string &filename) {
  ...
  if (particle["Dissociation energy, J"])
    diss_energy = particle["Dissociation energy, J"].as<vector<double>>();
  if (particle["Factor of symmetry"])
    rot_symmetry = particle["Factor of symmetry"].as<int>();
  if (particle["Parker (zeta^infty)"])
    parker_const = particle["Parker (zeta^infty)"].as<double>();
  if (particle["Reduced oscillator mass"])
    reduced_osc_mass = particle["Reduced oscillator mass"].as<double>();
  if (particle["Moment of Inertia, J*s^2"])
    rot_inertia = particle["Moment of Inertia, J*s^2"].as<double>();
  if (particle["Frequency of vibrations (we), m^-1"])
    vibr_frequency =
        particle["Frequency of vibrations (we), m^-1"].as<vector<double>>();
  if (particle["wexe, m^-1"])
    vibr_we_xe = particle["wexe, m^-1"].as<vector<double>>();
  if (particle["weye, m^-1"])
    vibr_we_ye = particle["weye, m^-1"].as<vector<double>>();
  if (particle["weze, m^-1"])
    vibr_we_ze = particle["weze, m^-1"].as<vector<double>>();
  if (particle["Be, m^-1"]) rot_be = particle["Be, m^-1"].as<vector<double>>();
  if (particle["ae, m^-1"]) rot_ae = particle["ae, m^-1"].as<vector<double>>();
  if (particle["internuclear distance, r_e , m"])
    internuclear_distance =
        particle["internuclear distance, r_e , m"].as<double>();
