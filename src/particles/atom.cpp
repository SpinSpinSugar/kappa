...
Atom::Atom(const std::string &name, const std::string &filename)
...
void Atom::readData(const std::string &name, const std::string &filename) {
  ...
  if (particle["Atomic_Number"])
    atomic_number = particle["Atomic_Number"].as<int>();  //!< atomic number
  if (particle["Atomic_Weight"])
    atomic_weight = particle["Atomic_Weight"].as<double>();  //!< atomic weight
}
