...
#define ARMA_DONT_PRINT_ERRORS  // to avoid warning message during solve()
                                // linear systems
...
class Mixture : public Approximation {
 ...
 public:
  ...
  Mixture(const kappa::Molecule &molecule, const kappa::Atom &atom,
          const std::string &interactions_filename = "interaction.yaml",
          const std::string &particles_filename = "particles.yaml");
  ...
  Mixture(const std::string particle_names,
          const std::string &interactions_filename = "interaction.yaml",
          const std::string &particles_filename = "particles.yaml",
          bool anharmonic = true, bool rigid_rotators = true);
  ...
  // Returns an array of the number of vibrational levels of molecules in the
  // mixture
  ...
  kappa::Interaction interaction(const kappa::Molecule &molecule1,
                                 const kappa::Molecule &molecule2);
  kappa::Interaction interaction(const kappa::Molecule &molecule,
                                 const kappa::Atom &atom);
  kappa::Interaction interaction(const kappa::Atom &atom,
                                 const kappa::Molecule &molecule);
  kappa::Interaction interaction(const kappa::Atom &atom1,
                                 const kappa::Atom &atom2);

  double debye_length(double T, const std::vector<arma::vec> &n_vl_molecule,
                      const arma::vec &n_atom, double n_electrons);
  double debye_length(double T, const std::vector<arma::vec> &n_vl_molecule,
                      double n_electrons);
  double debye_length(double T, const arma::vec &n_molecule,
                      const arma::vec &n_atom, double n_electrons);
  double debye_length(double T, const arma::vec &n);

  // Calculation of the number density of the mixture
  // @param const std::vector<arma::vec> &n_vl_molecule - the population of the
  // vibrational levels of molecules
  // @param const arma::vec &n_molecule                 - Vector of number
  // densities of molecules
  // @param const arma::vec &n_atom                     - Vector of number
  // densities of atoms
  // @param double n_electrons                          - the number density of
  // electrons (the default value is 0)
  // @param const arma::vec &n                          - Vector of number
  // densities
  double compute_n(const std::vector<arma::vec> &n_vl_molecule,
                   const arma::vec &n_atom, double n_electrons = 0.0);
  double compute_n(const arma::vec &n_molecule, const arma::vec &n_atom,
  ...
  arma::vec compute_density_array(const std::vector<arma::vec> &n_vl_molecule,
                                  ...
                                  double n_electrons = 0.0);
  arma::vec compute_density_array(const std::vector<arma::vec> &n_vl_molecule);
  double compute_density(const std::vector<arma::vec> &n_vl_molecule,
                         const arma::vec &n_atom, double n_electrons = 0.0);
  double compute_density(const std::vector<arma::vec> &n_vl_molecule,
                         double n_electrons = 0.0);
  double compute_density(const arma::vec &n_molecule, const arma::vec &n_atom,
                         double n_electrons = 0.0);
  double compute_density(const arma::vec &n);

  // Compute the pressure of the mixture
  double compute_pressure(double T, const std::vector<arma::vec> &n_vl_molecule,
                          const arma::vec &n_atom, double n_electrons = 0.0);

  // Calculation of the specific heat of translational degrees of freedom
  double c_tr(const std::vector<arma::vec> &n_vl_molecule,
              const arma::vec &n_atom, double n_electrons = 0.0);
  double c_tr(const std::vector<arma::vec> &n_vl_molecule,
              double n_electrons = 0.0);
  double c_tr(const arma::vec &n_molecule, const arma::vec &n_atom,
              double n_electrons = 0.0);
  double c_tr(const arma::vec &n);

  // Calculation of the specific heat of rotational degrees of freedom
  double c_rot(double T, const std::vector<arma::vec> &n_vl_molecule,
               const arma::vec &n_atom, double n_electrons = 0.0);
  double c_rot(double T, const std::vector<arma::vec> &n_vl_molecule,
               double n_electrons = 0.0);
  double c_rot(double T, const arma::vec &n_molecule, const arma::vec &n_atom,
               double n_electrons = 0.0);
  ...
  // void compute_transport_coefficients(double T, const arma::vec &n_molecule,
  // const arma::vec &n_atom, double n_electrons, kappa::models_omega
  // model=kappa::models_omega::model_omega_esa, double perturbation=1e-9);

  // void compute_transport_coefficients(double T, const arma::vec &n_molecule,
  // const arma::vec &n_atom, kappa::models_omega
  // model=kappa::models_omega::model_omega_esa, double perturbation=1e-9);
  ...
  // (for calculation it is necessary to call the function
  // compute_transport_coefficients)
  arma::mat get_diffusion();
  arma::mat get_lite_diffusion();
  arma::vec get_binary_diffusion();
  // arma::mat binary_diffusion(double T, kappa::models_omega
  // model=kappa::models_omega::model_omega_esa);
 ...
 protected:
  ...
  // if our mixture has no atoms, we pass this as a dummy parameter to functions
  // which expect numeric density of atomic species
  ...
  // this allows to easily calculate which elements of state-to-state matrix we
  // need to fill, given the molecule indices and the vibrational levels
  // vl_offset[0] = 0;
  // vl_offset[1] = molecules[0].num_vibr_levels[0];
  // vl_offset[2] = molecules[0].num_vibr_levels[0] +
  // molecules[1].num_vibr_levels[0], etc.
  ...
  void check_n_vl_molecule(
      const std::vector<arma::vec>
          &n_vl_molecule);  // check sizes of arrays and test
                            // values for non-negativity
  void check_n_molecule(
      const arma::vec &n_molecule);  // check sizes of arrays and test values
                                     // for non-negativity
  void check_n_atom(const arma::vec &n_atom);
  void check_n(const arma::vec &n);

  void compute_omega11(double T, kappa::models_omega model =
                                     kappa::models_omega::model_omega_esa);
  void compute_omega12(double T, kappa::models_omega model =
                                     kappa::models_omega::model_omega_esa);
  void compute_omega13(double T, kappa::models_omega model =
                                     kappa::models_omega::model_omega_esa);
  void compute_omega22(double T, kappa::models_omega model =
                                     kappa::models_omega::model_omega_esa);

  // Particle + e- interactions
  void compute_omega11(
      double T, double debye_length,
      kappa::models_omega model = kappa::models_omega::model_omega_esa);
  void compute_omega12(
      double T, double debye_length,
      kappa::models_omega model = kappa::models_omega::model_omega_esa);
  void compute_omega13(
      double T, double debye_length,
      kappa::models_omega model = kappa::models_omega::model_omega_esa);
  void compute_omega22(
      double T, double debye_length,
      kappa::models_omega model = kappa::models_omega::model_omega_esa);
  ...
  const arma::mat &compute_bulk_viscosity_LHS(
      double T,
      kappa::models_omega model = kappa::models_omega::model_omega_esa);
  const arma::vec &compute_bulk_viscosity_RHS(double T);
  const arma::vec &compute_bulk_viscosity_coeffs(
      double T,
      kappa::models_omega model = kappa::models_omega::model_omega_esa);

  const arma::mat &compute_bulk_viscosity_rigid_rot_LHS(
      double T,
      kappa::models_omega model = kappa::models_omega::model_omega_esa);
  const arma::vec &compute_bulk_viscosity_rigid_rot_RHS(double T);
  const arma::vec &compute_bulk_viscosity_rigid_rot_coeffs(
      double T,
      kappa::models_omega model = kappa::models_omega::model_omega_esa);

  const arma::mat &compute_thermal_conductivity_LHS(
      double T,
      kappa::models_omega model = kappa::models_omega::model_omega_esa);
  const arma::vec &compute_thermal_conductivity_RHS(double T);
  const arma::vec &compute_thermal_conductivity_coeffs(
      double T,
      kappa::models_omega model = kappa::models_omega::model_omega_esa);

  const arma::mat &compute_thermal_conductivity_rigid_rot_LHS(
      double T,
      kappa::models_omega model = kappa::models_omega::model_omega_esa);
  const arma::vec &compute_thermal_conductivity_rigid_rot_RHS(double T);
  const arma::vec &compute_thermal_conductivity_rigid_rot_coeffs(
      double T,
      kappa::models_omega model = kappa::models_omega::model_omega_esa);

  // const arma::mat &compute_diffusion_LHS(double T, kappa::models_omega
  // model=kappa::models_omega::model_omega_esa); const arma::vec
  // &compute_diffusion_LHS(double T); const arma::vec
  // &compute_diffusion_RHS(double T); const arma::vec
  // &compute_diffusion_RHS(double T, int b, int n); const arma::vec
  // &compute_diffusion_coeffs(double T, kappa::models_omega
  // model=kappa::models_omega::model_omega_esa);
  const arma::mat &compute_diffusion_LHS(double T);
  const arma::mat &lite_compute_diffusion_LHS(double T);
  const arma::mat &lite_compute_diffusion_LHS(double T, int b, int n,
                                              std::string ParticleType);
  const arma::vec &compute_diffusion_RHS(double T, int b, int n);
  const arma::vec &lite_compute_diffusion_RHS(double T, int b, int n,
                                              std::string ParticleType);
  const arma::vec &compute_diffusion_coeffs(double T, int b, int n);
  const arma::vec &lite_compute_diffusion_coeffs(double T, int b, int n,
                                                 std::string ParticleType);
  // const arma::mat &compute_diffusion_rigid_rot_LHS(double T,
  // kappa::models_omega model=kappa::models_omega::model_omega_esa); const
  // arma::vec &compute_diffusion_rigid_rot_RHS(double T); const arma::vec
  // &compute_diffusion_rigid_rot_coeffs(double T, kappa::models_omega
  // model=kappa::models_omega::model_omega_esa);

  const arma::mat &compute_shear_viscosity_LHS(
      double T,
      kappa::models_omega model = kappa::models_omega::model_omega_esa);
  const arma::vec &compute_shear_viscosity_RHS(double T);
  const arma::vec &compute_shear_viscosity_coeffs(
      double T,
      kappa::models_omega model = kappa::models_omega::model_omega_esa);

  double bulk_viscosity(double T, kappa::models_omega model =
                                      kappa::models_omega::model_omega_esa);
  double thermal_conductivity(
      double T,
      kappa::models_omega model = kappa::models_omega::model_omega_esa);
  double shear_viscosity(double T, kappa::models_omega model =
                                       kappa::models_omega::model_omega_esa);

  void diffusion(double T);
  void lite_diffusion(double T);

  void thermodiffusion(double T, kappa::models_omega model =
                                     kappa::models_omega::model_omega_esa);
  void binary_diffusion(double T, kappa::models_omega model =
                                      kappa::models_omega::model_omega_esa);
